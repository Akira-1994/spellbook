...
第 1149–1162 頁的召喚附錄不在 A–Z 詳述範圍內，由 `scripts/add_summon_spells.py` 另外加入 32 個召喚術：召喚怪物術一～九級、召喚自然盟友術一～九級、召喚亡靈一～五級（死者之書）、召喚沙漠盟友一～九級（沙暴之書）。

- 二級以上的原文只寫出與一級不同之處，未寫出的規則欄位（成分、施法時間、距離、持續時間、豁免、法術抗力）取自同系列一級法術。
- 各系列的召喚生物列表依等級放進對應法術的說明末尾，附註（水中限定、新怪物）只附在含有該標記的列表。召喚亡靈的列表在 PDF 中是五欄表格，依原書所引的萬法大全列表整理。
- 九級召喚怪物術的「守序 9」依職業對照記為秩序領域。
- `spell_entries` 的頁碼限制因此放寬到 1162（migration 5，重建資料表）。

第 1145–1148 頁的降咒附錄是規則說明，沒有法術，不收錄。
